 ...
 * Example #2: Given I am a logged in user with the "SEO admin" user
 ...
 * Example #2: Then the response status of "/admin/config/search/metatag" should be 200
 */
 ...
 *
 * Example #1: Then the response body of "/sitemap.xml" should contain "<urlset"
 * Example #2: Then the response body of "/robots.txt" should contain "Sitemap:"
 */
...
/**
 * Assert that the JSON-LD schema graph contains a node of a given @type.
 *
 * Example #1: Then the JSON-LD on the page should contain a "WebSite" node
 * Example #2: Then the JSON-LD on the page should contain a "WebPage" node
 */
Then(/^the JSON-LD on the page should contain a "([^"]+)" node$/, async function (type) {
  await attempt(async () => {
        ...
        for (const item of items) {
          ...
          if (Array.isArray(item['@graph'])) {
            for (const g of item['@graph']) {
              const gt = g['@type'];
              if (gt === type || (Array.isArray(gt) && gt.includes(type))) {
                found = true;
              }
            }
          }
    ...
    if (!found) {
      throw new Error(`No JSON-LD node with @type="${type}" found`);
    }
  }, `Expected JSON-LD on the page to contain a "${type}" node`);
});
